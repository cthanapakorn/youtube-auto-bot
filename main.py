...
def download_image_final(args):
    """วาดภาพเล่าเรื่องคุณภาพสูง (Flux) เน้นความต่อเนื่อง"""
    prompt, filename, scene_num = args
    print(f"   🎨 ฉากที่ {scene_num}: กำลังเนรมิตภาพประกอบ...")
    clean_p = re.sub(r'[^\w\s]', '', prompt).strip().replace(' ', '%20')
    # เพิ่มรายละเอียดให้ภาพดูน่าดึงดูดและสอดคล้องกับเนื้อหา
...
def run_workflow():
    try:
        ...
        print("🧠 1. Gemini กำลังเขียนบทมหากาพย์ 60 วินาทีแบบสรุปจบในตัว...")
        # สั่งให้ AI เขียนบทพากย์ที่ยาวพอดีกับ 3 วินาทีต่อท่อน
        prompt_sys = (
            "จงสวมบทบาทเป็นนักเล่าเรื่องการเงินระดับโลก สร้างวิดีโอ Shorts หัวข้อ: 'ทำไมคุณขยันแต่ไม่รวย? ความลับดอกเบี้ยทบต้น' "
            "ภารกิจ: สร้างสคริปต์ 20 ท่อน ท่อนละ 3 วินาที (ต้องยาวประมาณ 25-30 คำต่อท่อนเพื่อให้พอดีเวลา) "
            "กฎเหล็ก:\n"
            "1. บทพากย์ (text): ภาษาไทยแนวเล่าเรื่อง มีจุดดึงดูดและสรุปจบใน 60 วินาที ห้ามขาดตอน\n"
            "2. คำสั่งวาดรูป (prompt): ภาษาอังกฤษที่อธิบาย 'เหตุการณ์และอารมณ์' (สลับภาพเงิน, นาฬิกาทราย, กราฟ, และตัวละคร)\n"
        ...
        print("🎙️ 2. สร้างเสียงพากย์คุณนิวัฒน์ (ชาย) แบบเน้นอารมณ์...")
        # รวมบทพูดและใส่จุดเว้นวรรคเพื่อให้พากย์ได้สมจริง
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            list(executor.map(download_image_final, tasks))

        print("🎬 4. ประกอบวิดีโอ 60 วินาที (แก้เรื่องฟอนต์และเพลงประกอบ)...")
        
        # ⚠️ จุดสำคัญ: ถ้ามีไฟล์ font.ttf เราจะใช้ชื่อฟอนต์มาตรฐานที่ FFmpeg เข้าใจ
        font_path = "font.ttf"
        ...
        with open("subs.ass", "w", encoding="utf-8-sig") as f:
            ...
            # ใส่ Fontname เป็น 'Sans' หรือชื่อฟอนต์ทั่วไป แล้วเราจะบังคับผ่าน FFmpeg อีกที
            f.write(f"Style: PopUp,Sans,150,&H0000E6FF,&H00FFFFFF,&H00000000,&H60000000,-1,0,0,0,100,100,0,0,1,18,12,2,80,80,450,1\n\n")
        ...
        # 📌 FFmpeg: ผสมเสียง + ซูม Cinematic + ฝังฟอนต์ไทยจากไฟล์โดยตรง
        ...
        # ใช้ subtitles filter พร้อมระบุ fontsdir เพื่อดึง font.ttf มาใช้งาน
        cmd = (
        ...
        if os.path.exists('token.json'):
            ...
            print("✨ ภารกิจมหากาพย์สำเร็จ! วิดีโอ 60 วินาทีคุณภาพสูงพร้อมใช้งาน!")
